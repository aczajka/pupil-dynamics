...
tab = readtable('../scores/scores_tab1_dataDrivenSection.csv');
...
VARIANT{1} = 'RNN';
VARIANT{2} = 'GRU';
VARIANT{3} = 'classic_LSTM';
VARIANT{4} = 'peephole_LSTM';
...
for r=1:length(STIMULUS)
    for t=1:length(VARIANT)
        ...
        tab_data = tab(ind,:);
        APCER = 100*abs(sum(tab_data.Node0 < tab_data.Node1) / height(tab_data));
        ...
        tab_data = tab(ind,:);
        BPCER = 100*(1-abs(sum(tab_data.Node0 < tab_data.Node1) / height(tab_data)));
